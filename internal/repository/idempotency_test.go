...
func (p *idempotencyRepositoryProvider) BranchExists(
	_ context.Context,
	_ string,
	_ string,
	_ string,
...
func TestService_Create_SkipsInitialPushWhenRepositoryAlreadyHasBranch(
	t *testing.T,
) {
	templateService := &idempotencyTemplateService{
		files: []templatepkg.TemplateFile{
...
`,
	...
	if repositoryProvider.branchExistsCalls != 1 {
		t.Errorf(
			"BranchExists() calls = %d, want 1",
			repositoryProvider.branchExistsCalls,
		)
	}
